...
def ventas(request):
    ...
    # Variable de contexto para enviar historial de ventas
    context = {
        'historial': historial,
    }
...
def usuarios(request, accion, id):
    if request.method == 'POST':
        user_form = UsuarioForm(request.POST, instance=get_object_or_404(User, id=id) if id != '0' else None)
        perfil_form = PerfilForm(request.POST, request.FILES, instance=get_object_or_404(Perfil, usuario_id=id) if id != '0' else None)
        ...
        if accion == 'eliminar':
            ...
            user_form = UsuarioForm(instance=get_object_or_404(User, id=id) if id != '0' else None)
            perfil_form = PerfilForm(instance=get_object_or_404(Perfil, usuario_id=id) if id != '0' else None)
...
def bodega(request):
    ...
    if request.method == 'POST':
        ...
        producto_id= request.POST_get('producto')
        producto = Producto.objects.get(id=producto_id)
        cantidad = int(request.POST_get('cantidad'))
        for cantidad in range(1, cantidad + 1):
            Bodega.objects.create(producto=producto)
        if cantidad  == 1:
            messages.success(request, f'se ha agregado un nuevo "{producto.nombre}" a la bodega')
        else:
             messages.success(request, f'se ha agregado "{cantidad}" productos de "{producto.nombre}" a la bodega')
...
def eliminar_producto_en_bodega(request, bodega_id):
    # Obtener el objeto de la bodega a eliminar
    bodega = get_object_or_404(Bodega, id=bodega_id)
    
    # Eliminar la bodega
    bodega.delete()

    # Redirigir de vuelta a la página de bodega
    return redirect('bodega')

@login_required
@user_passes_test(es_cliente_autenticado_y_activo)
def miscompras(request):
    # es_cliente_autenticado_y_activo
    ...
    # Variable de contexto para enviar historial de compras
    context = {
        'historial': historial,
    }
